...
function parsePlantel(text){
  ...
  p.data.forEach(raw=>{
    ...
    const nombre=g('nombre'), dorsal=g('dorsal'), rol=asRole(g('rol'),esSi(g('portero')))
    if(!nombre&&!dorsal) return
    if(!STAFF.includes(rol)&&!dorsal){issues.push(nombre);return}
    rows.push({id:g('id'),nombre,apodo:g('apodo'),dorsal,rol,
...
function buildPlantel(rows,meta){
  ...
  return ['# ardi:plantel v1',`# club: ${meta.club}`,`# clubId: ${meta.clubId}`,
    ...
    ...rows.map(r=>{const b=[r.id,r.nombre,r.apodo,r.dorsal,r.rol,r.isGoalie?'si':'no']
...
const original=[
  {id:'ILE-0001',nombre:'Keily Lorca',      apodo:'Keily', dorsal:'19',rol:'portero',isGoalie:true, doc:''},
  {id:'ILE-0007',nombre:'Pascale Celis',    apodo:'Pascu', dorsal:'7', rol:'jugador',isGoalie:false,doc:''},
  {id:'ILE-0020',nombre:'Génesis Cárdenas', apodo:'',      dorsal:'85',rol:'jugador',isGoalie:false,doc:''},
  {id:'ILE-0031',nombre:'Facundo Oyola',    apodo:'',      dorsal:'',  rol:'dt',     isGoalie:false,doc:''},
]
...
const liga=`Número,Nombre completo,Rol
19,Keily Lorca,Portero
7,Pascale Celis,Jugador`
const r2=parsePlantel(liga).rows
check(r2.length===2,'lee un archivo con cabeceras acentuadas',`${r2.length}`)
check(r2[0].dorsal==='19','reconoce "Número" con tilde (hoy falla y usa la 1a columna)')
check(r2[0].nombre==='Keily Lorca','reconoce "Nombre completo"')

const alt=`Dorsal;Jugadora;Alias
19;Keily Lorca;Keily`
